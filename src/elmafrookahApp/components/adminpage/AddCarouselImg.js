...
import { showPost, destroyPostCrImge, createImgOrVid } from "../../api";
...
const AddCarousel = props => {
  ...
  const imge = post.carouselimge
    ? `${post.carouselimge}`
    : logoicon
    ? logoicon
    : !post.carouselimge || !logoicon
    ? setLoadSpiner(true)
    : setLoadSpiner(false);
  ...
  const destroyImg = () => {
    setLoadSpiner(true);
    setIsRemoved(true);
    const destImg = storage.refFromURL(`${post.carouselimge}`);
    destImg
      .delete()
      .then(() => {
  ...
  const handleImgSubmit = event => {
    ...
    const uploadTask = storage
      .ref(`images/cr${postid}${imgFile.name}`)
      .put(imgFile);
    // Firbase storage
